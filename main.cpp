...
int main( int argc, char *argv[] )
{
    ...
    if ( ( argc > 2 ) && ( zdsp1d->SetServerNr(argv[2]) ) )
    { //  neue dsp karten nummer
        syslog(LOG_EMERG,"illegal pcb nr\n") ;
        r=1;
    }
    
    if ( ( argc > 3) && ( zdsp1d->SetDeviceNode(argv[3]) ) )
    { // neuer device knoten
        syslog(LOG_EMERG,"illegal dsp device node\n");
        r=1;
    }
    
    if ( ( argc > 4) && ( zdsp1d->SetBootPath(argv[4]) ) )
    { // neuer boot path
        syslog(LOG_EMERG,"illegal dsp boot path\n");
        r=1;
    }
    
    if (zdsp1d->DspDevOpen() < 0)
    { // versuchen wir hier das dsp device zu öffnen
        syslog(LOG_EMERG,"dsp device not available\n");
        r=1;
    }

    if (r==0)
    { // nur wenn bis jetzt kein fehler aufgetreten ist
#ifndef DEBUG
